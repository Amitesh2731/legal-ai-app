...
import { logOutOutline, createOutline, personCircleOutline, mailOutline, callOutline, locationOutline } from 'ionicons/icons';
...
@Component({
      ...
      <div class="profile-container">
        ...
        <div class="profile-info">
          <div class="info-item">
            ...
            <div><label>Email</label><p>{{ user?.email || 'Not set' }}</p></div>
          </div>
          <div class="info-item">
            <ion-icon name="call-outline"></ion-icon>
            <div><label>Phone</label><p>{{ user?.phone || 'Not set' }}</p></div>
          </div>
          <div class="info-item">
            <ion-icon name="location-outline"></ion-icon>
            <div><label>Location</label><p>{{ user?.city ? user.city + ', ' + user.state : 'Not set' }}</p></div>
          </div>
        </div>
  ...
  styles: [`
    ...
    .profile-role { font-size: 14px; color: rgba(255,255,255,0.6); margin: 0; text-transform: capitalize; }
...
export class ClientProfilePage {
  ...
  constructor(private authService: AuthService) {
    addIcons({ logOutOutline, createOutline, personCircleOutline, mailOutline, callOutline, locationOutline });
